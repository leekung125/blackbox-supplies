 ...
 * highest-value query shape available to a low-authority site. The autocomplete-both-directions and
 * weak-SERP claims that stood here were the writer's untested assumption, not a measured finding, and
 * have been removed rather than dressed up: no SERP or autocomplete capture exists in this repo to
 * back them. The commercial case above stands on the catalog alone. Deliberately NOT written as the
 * brand-level
 ...
 * 3,300W figure in lib/articles/best-power-station-apartment-power-outage.ts. BOTH WERE CORRECTED to
 * 3,000W in the 2026-08-26 audit, along with the catalog's '2x USB-C 100W' keySpec, which Jackery's
 * table contradicts the same way ('(1x) 30W Max' and '(1x) 100W Max'). Related: our catalog says the Jackery has "2x USB-C 100W"; Jackery's spec
 ...
 *
 * FIXED IN AUDIT (2026-08-26), both by fetching the two cited pages and reading them:
 * - The draft called BOTH units "pure sine wave". BLUETTI publishes "Inverter Type: Pure Sine Wave";
 *   jackery.com never uses the phrase (every "sine" match on that page is inside the word "Business")
 *   and its spec table has no inverter-type row at all. Now attributed to BLUETTI only.
 * - The draft said BLUETTI "markets a UPS mode on the AC180". The string "UPS" does not occur on
 *   bluettipower.com/products/ac180, nor does "switchover", "uninterruptible" or any ms figure. That
 *   was a label inferred from our catalog, not read off the page. The page publishes exactly one line
 *   on the subject - "Pass-through Charging: Yes" - and that is now all this article claims for it.
 */
    ...
    },
    {
      ...
      table: {
        ...
        rows: [
          ...
          ["Full-size fridge, compressor start", "Brief inrush, roughly 1,000-1,800W, then settles", "Inside its 3,000W surge", "Inside its 2,700W surge"],
          ["Large side-by-side, compressor start", "Can touch about 2,400W", "Comfortable", "Close to its 2,700W ceiling"],
