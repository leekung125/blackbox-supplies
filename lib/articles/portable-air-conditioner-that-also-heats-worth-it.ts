 ...
 *
 * ADVERSARIAL AUDIT 2026-08-26 - every source URL refetched, every number re-derived. WHAT HELD:
 * all five source URLs resolve and say what their labels claim (the ecoflow.com/us/wave-3 path that
 * 404s elsewhere in this batch is NOT used here); EcoFlow's page really does carry "6100 BTU cooling",
 * "6800 BTU heating", "It's Also a Heater", both "Tested in a 10 cubic meter space" footnotes at 77F
 * and 60F, "Operates at just 44 dB", the 1024Wh LFP add-on, "8 hours of wireless use in Eco Mode under
 * optimal conditions", the "Water Drain Alert" wording and the "insulated exhaust duct", and the phrase
 * "heat pump" genuinely does NOT appear on it; both Midea pages exist and carry the quoted titles, the
 * "Heat No" row, the contradicting "Comfort Features Cool, Dehumidify, Ventilate, Heat" row, 42 dB,
 * 550 sq ft and hose-in-hose; the Amazon title for B0FC2SGGF9 matches word for word. Both ABSENCE
 * claims were verified as absences, not assumed: neither manufacturer page states a minimum operating
 * temperature. All arithmetic recomputed (3.412, 5118, 6140, 1993, 128 W, 797 W, ~75 min, 353 cu ft,
 * 1152 cu ft, 12.5x) and correct.
 *
 * WHAT DID NOT HOLD, and was fixed in place:
 *   1. INVENTED "NOT PUBLISHED" BLANK - the Midea comparison table said MPT1412HVRU's weight was
 *      "Not published on the page". It is published: "Product Weight (lbs) 77.16 lbs". Same defect
 *      species as the sibling whynter-arc-1230wn-vs-midea-duo draft, which invented a missing hose
 *      count. A blank asserted without opening the page is a fabrication with better manners.
 *   2. UNSOURCED MAGNITUDE - "a factor of two or three in running cost" / "typically delivers two to
 *      three units of heat energy" / table cells "Typically 2-3" and "Roughly a third to a half".
 *      No COP is published for either unit (the page says so itself) and no source was cited. The
 *      structural twin of this batch's "can differ BY A FACTOR OF THREE". Now states the direction
 *      (above 1.0, which is definitional) and refuses the magnitude; the 2.5 figure survives only
 *      inside an explicitly conditional "if a unit ran at" sentence.
 *   3. A CONVENTION SOLD AS PHYSICS - "electrical code limits continuous loads to 80% of a circuit's
 *      rating. That is why essentially every plug-in space heater is a 1,500-watt device". 80% of
 *      1,800 W is 1,440 W, not 1,500 W, so the stated cause cannot produce the stated number; and
 *      "That is a hard ceiling, not a convention" was backwards - 5,118 BTU/h IS the market
 *      convention. The circuit's real hard ceiling is 1,800 W = ~6,140 BTU/h. Both figures are now
 *      given and labelled correctly, which makes the WAVE 3 diagnosis STRONGER: 6,800 BTU/h = 1,993 W
 *      exceeds even the absolute circuit maximum.
 *   4. UNVERIFIABLE ASSERTION - "They photograph identically." Replaced with the checkable fact that
 *      Midea publishes identical dimensions for both SKUs (32.48 x 19.53 x 16.73).
 *   5. HANDS-ON AMBIGUITY - "Run both tests on the EcoFlow WAVE 3" reads as bench testing. Now
 *      "Apply both tests ... on paper, against the published figures".
 *   6. A REASONING HOLE THE DRAFT DID NOT ADMIT - the whole outlet-ceiling test assumes a mains-only
 *      unit, and the WAVE 3 is battery-capable, so the outlet limit constrains it less than the draft
 *      implied. Now stated outright in both the body and the FAQ, with the note that the
 *      heating-exceeds-cooling tell is independent of it.
 *   7. ESTIMATES DRESSED AS DATA - the "Weeks of heat use per year" column is editorial judgement.
 *      Caption and header now say so.
 *
 * STILL OPEN, NOT THIS FILE'S TO CLOSE (both would change live pages, and both need a human call):
 *   a. data/heat-products.json (see the note above) keys "inverter cool + heat", "a heat mode for
 *      cooler months" and "a heat-pump assist" to model MAP14S1TBL, which Midea spec-sheets "Heat No"
 *      - while the ASIN we actually sell, B0FC2SGGF9, is titled "with Heat" twice on Amazon. Picking
 *      a side needs evidence we do not have; this page deliberately reports both strings instead.
 *      Repeated in prose in do-swamp-coolers-work-in-humid-climates.ts and
 *      single-hose-vs-dual-hose-portable-ac.ts.
 *   b. lib/articles/is-the-ecoflow-wave-3-worth-it.ts (also not yet wired in) answers "Is the EcoFlow
 *      WAVE 3 a heat pump?" with a flat "Yes ... the WAVE 3 is a reversible one". EcoFlow's page never
 *      says that. If both pages ship, they contradict each other; this one holds the defensible line.
 */
  ...
  sections: [
    {
      ...
      table: {
        ...
        rows: [
          ...
          ["Heat per watt of electricity", "Exactly 1.0 — the physical ceiling", "Above 1.0. Neither maker publishes the figure"],
          ["Running cost for the same warmth", "Highest possible", "Lower, by an amount nobody has published"],
          ["Output on a standard US outlet", "~5,100 BTU/h in practice; 6,140 is the circuit's limit", "Can exceed both — the heat is fetched, not made"],
    ...
    },
    {
      ...
      table: {
        ...
        rows: [
          ...
          ["Heating output well above ~5,100 BTU/h", "Almost certainly reverse-cycle; certain above ~6,140", "This is the version worth money"],
    ...
    },
    {
      ...
      table: {
        ...
        rows: [
          ...
          ["Listed weight", "73.85 lbs", "77.16 lbs"],
        ],
      },
    },
    {
    ...
    },
    {
      ...
      table: {
        caption: "When the heating premium actually earns its money — the week counts are our estimates, not measured figures",
        columns: ["Your situation", "Rough weeks of heat use a year", "Verdict"],
        rows: [
