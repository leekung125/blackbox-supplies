        ...
        {picks.map((p, i) => {
          ...
          return (
            <Link
              key={p.id}
              href={`/products/${p.id}`}
              className="group flex items-center gap-4 px-5 py-4 transition-colors hover:bg-surface-2 sm:px-7"
            >
              ...
              <div className="shrink-0 text-right">
                ...
                <div className="mono mt-0.5 text-[0.56rem] uppercase tracking-[0.08em] text-ink-faint">{field?.label ?? "spec"}</div>
              </div>
            </Link>
          );
        })}
